...
Developer = (function() {
	...
	$.extend(true, Developer, {
		...
		execTests: function() {
			...
			function runJasmine() {
				if (!self.jasmineEnv) {
					self.jasmineEnv = jasmine.getEnv();
					self.jasmineEnv.updateInterval = 1000;

					self.jasmineEnv.addReporter(new jasmine.ConsoleReporter());
				}

				self.jasmineEnv.execute();
			}

			// jasmine.js ya no se carga en el arranque de producción (era un
			// framework de testing de ~76KB que descargaba y parseaba cada TV
			// sin usarlo casi nunca). Se carga bajo demanda solo la primera vez
			// que se abre este menú de developer (reacción a la tecla 6).
			if (typeof jasmine === 'undefined') {
				var script = document.createElement('script');
				script.src = 'js/core/jasmine.js';
				script.onload = function() {
					if (typeof jasmine === 'undefined') {
						console.error('No se pudo cargar jasmine.js bajo demanda.');
						return;
					}
					runJasmine();
				};
				script.onerror = function() {
					console.error('Error al cargar jasmine.js bajo demanda.');
				};
