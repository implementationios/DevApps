		...
		init: function(config) {
			...
			if (typeof Control !== 'undefined') {
				Control.on('beforekey', function(keyCode, event) {
					// Los inputs con clase "app-keyboard" (login, búsqueda) ya no
					// aceptan escritura directa: son readonly y el texto se escribe
					// exclusivamente a través del teclado propio (Keyboard), así que
					// este fallback de "anexar números directo" se salta para ellos.
					if (this.focused && this.focused[0].nodeName === 'INPUT' && !this.focused.hasClass('app-keyboard')) {
