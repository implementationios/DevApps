 ...
 */
 ...
 Snippet = (function (Events) {
    ...
    $.extend(true, Snippet.prototype, Events, {
		 ...
		 */
        desctruct: function () {
            // Antes: unbindEvents() existía pero nunca se llamaba desde aquí,
            // dejando el fix "a medias" — los listeners de cada snippet
            // (carruseles, filas, items de grilla, que se crean/destruyen con
            // mucha más frecuencia que las Scenes) nunca se liberaban.
            this.unbindEvents();
        ...
        unbindEvents: function () {
            ...
            // Debe reflejar exactamente la misma rama que initEvents() usó al
            // registrar (this.parent vs. globales Control/Mouse), si no,
            // los snippets con parent nunca se llegaban a desligar de él.
            if (this.parent) {
                this.parent.off('key', this.onKeyDown, this);
                this.parent.off('click', this.eventsArray[0], this);
                this.parent.off('scroll', this.eventsArray[1], this);
            } else {
